...
namespace MarketBook.Domain.Watchlist.ValueObjects;

/// <summary>
/// EN: Represents the unique identifier of a Market.
/// FA: شناسه یکتای Market را نمایش می‌دهد.
/// </summary>
public sealed record WatchlistId : EntityId
{
    /// <summary>
    /// EN: Initializes a new instance of the <see cref="WatchlistId"/> class.
    /// FA: نمونه جدیدی از کلاس <see cref="WatchlistId"/> را ایجاد می‌کند.
    /// </summary>
    /// <param name="value">Underlying ULID value.</param>
    private WatchlistId(Ulid value) : base(value)
    {
    }
    ...
    /// </summary>
    public static WatchlistId New() => new(Ulid.NewUlid());

    /// <summary>
    /// EN: Creates an identifier from an existing ULID.
    /// FA: یک شناسه از یک ULID موجود ایجاد می‌کند.
    /// </summary>
    public static WatchlistId FromUlid(Ulid value) => new(value);

    /// <summary>
    /// EN: Parses a ULID string into an identifier.
    /// FA: رشته ULID را به شناسه تبدیل می‌کند.
    /// </summary>
    public static WatchlistId Parse(string value)
    {
        Guard.AgainstNullOrWhiteSpace(value, nameof(WatchlistId));
        ...
        throw new DomainException(
            new Error(
                $"{nameof(WatchlistId)}.InvalidFormat",
    ...
    /// </summary>
    public static bool TryParse(string? value, out WatchlistId? result)
    {
    ...
    /// </summary>
    public static explicit operator WatchlistId(string value) => Parse(value);

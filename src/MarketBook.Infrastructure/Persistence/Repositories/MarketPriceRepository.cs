...
/// <inheritdoc/>
public Task<MarketPrice?> GetLatestByListingIdAsync(ListingId listingId,DateOnly onOrBeforeTradingDate,CancellationToken cancellationToken=default)=>_dbContext.Set<MarketPrice>().AsNoTracking().Where(item=>item.ListingId==listingId && item.TradingDate<=onOrBeforeTradingDate).OrderByDescending(item=>item.TradingDate).ThenByDescending(item=>item.CreatedOn).FirstOrDefaultAsync(cancellationToken);
/// <inheritdoc/>
